...
import { markApiCallPaid, getPendingApiCalls } from "@/db/actions/mcp-actions";
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Check for pending API calls and mark them as paid (simplified for demo)
          // In production, you would verify actual blockchain transactions
          const pendingCalls = await getPendingApiCalls(fid);
          for (const call of pendingCalls) {
            await markApiCallPaid(call.id, "simulated-transaction-hash");
          }
        } catch (err) {
